...
namespace AFKS.Core.Systems
{
    ...
    /// </summary>
    [AddComponentMenu("AFKS/Core/Global Settings Service")]
    public sealed class GlobalSettingsService : MonoBehaviour
    {
        [Header("설정창 UI")]
        [SerializeField]
        [InspectorName("설정 패널")]
        ...
        private void Awake()
        {
            Debug.Log("GlobalSettingsService 초기화");
            // ServiceLocator에 등록하여 어디서든 조회 가능하게 함
            AFKS.Core.Services.ServiceLocator.Register<GlobalSettingsService>(this, overwriteExisting: true);
        ...
        {
            AFKS.Core.Services.ServiceLocator.Unregister<GlobalSettingsService>();
        }
        ...
        private void Update()
        {
            ...
            {
                Debug.Log("GlobalSettingsService: ESC 키가 눌렸습니다!");
        ...
        public void ResetSave()
        {
            ...
            if (shouldReset)
            {
                ...
                {
                    sceneService.LoadScene("Core", LoadSceneMode.Single);
                }
                else
                {
                    // 폴백: 직접 SceneManager 사용
