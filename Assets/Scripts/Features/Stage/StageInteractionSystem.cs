...
{
    /// <summary>
    /// 스테이지의 상호작용 가능한 오브젝트들을 제어하는 시스템입니다.
    /// 각 이벤트에 따라 클릭 가능한 오브젝트를 동적으로 제어합니다.
    /// </summary>
    [AddComponentMenu("AFKS/Stage/Stage Interaction System")]
    public class StageInteractionSystem : MonoBehaviour
    {
        ...
        public static StageInteractionSystem Instance { get; private set; }
        ...
        private void InitializeInteractableObjects()
        {
            ...
            Debug.Log($"StageInteractionSystem: 초기화 시작 - {interactableObjects.Count}개 오브젝트");
            ...
            for (int i = 0; i < interactableObjects.Count; i++)
            {
                ...
                if (!idToInteractable.ContainsKey(io.objectId))
                {
                    ...
                    Debug.Log($"StageInteractionSystem: {io.objectId} 등록됨 - {io.gameObject.name}");
                }
            }
            
            Debug.Log($"StageInteractionSystem: 초기화 완료 - 등록된 오브젝트: {string.Join(", ", idToInteractable.Keys)}");
        ...
        public GameObject GetObject(string objectId)
        {
            if (TryGet(objectId, out var io))
            {
                ...
                {
                    Debug.Log($"StageInteractionSystem.GetObject: {objectId} 발견 - {io.gameObject.name}");
                    return io.gameObject;
                }
                else
                {
                    Debug.LogWarning($"StageInteractionSystem.GetObject: {objectId}의 gameObject가 null입니다.");
                }
            }
            else
            {
                Debug.LogWarning($"StageInteractionSystem.GetObject: {objectId}를 찾을 수 없습니다. 등록된 오브젝트: {string.Join(", ", idToInteractable.Keys)}");
            }
            return null;
        }
